...
public class ThemeService implements IThemeService {
    ...
    public ThemeDTO save(ThemeDTO theme) {
        ...
        CategoryEntity category = categoryRepository.findOne(theme.getCategoryId());
    ...
    public List<ThemeDTO> getAllTheme() {
        ...
        List<ThemeEntity> listEntity = themeRepository.findAll();
        for (ThemeEntity theme : listEntity) {
            listDto.add(modelMapper.map(theme, ThemeDTO.class));
        }
        return listDto;
    }

    @Override
    public ThemeDTO getOneThemeById(long id) {
        ThemeEntity themeEntity = themeRepository.findOne(id);
        return modelMapper.map(themeEntity, ThemeDTO.class);
    }

    @Override
    public ThemeDTO getThemeByName(String name) {
        ThemeEntity themeEntity = themeRepository.findOneByName(name);
        return modelMapper.map(themeEntity, ThemeDTO.class);
    }

    @Override
    public List<ThemeDTO> getThemeByCode(String code) {
        List<ThemeDTO> listDto = new ArrayList<>();
        List<ThemeEntity> listEntity = themeRepository.getThemeByCode(code);
        for (ThemeEntity theme : listEntity) {
            listDto.add(modelMapper.map(theme, ThemeDTO.class));
        }
        return listDto;
